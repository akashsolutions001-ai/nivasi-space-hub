...
import { n as useAuth } from "./auth-DCFmY9CZ.mjs";
import { t as useQuery } from "../_libs/tanstack__react-query.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/hooks-BH6AsUXG.js
...
function mapMess(snap) {
	...
	return {
		...
		messName: d.messName ?? "",
		serialNumber: typeof d.serialNumber === "number" ? d.serialNumber : void 0,
		ownerName: d.ownerName ?? "",
		ownerPhone: d.ownerPhone ?? "",
		propertyId: d.propertyId ?? "",
...
async function unassignStudentFromLaundry(admissionDocId) {
	try {
		await updateDoc(doc(getDb(), "admissions", admissionDocId), {
			laundryId: "",
			laundryName: "",
...
async function updateStudentLaundryStatus(admissionDocId, laundryStatus) {
	try {
		await updateDoc(doc(getDb(), "admissions", admissionDocId), {
			laundryStatus: laundryStatus || "active",
			updatedAt: serverTimestamp()
...
async function fetchLaundryPickupsForStudent(studentId) {
	try {
		return (await getDocs(query(collection(getDb(), "laundryPickups"), where("studentId", "==", studentId)))).docs.map(mapLaundryPickup).sort((a, b) => b.date.localeCompare(a.date));
	} catch (error) {
		...
		if (!existing.empty) {
			...
			if (existingDoc) {
				await updateDoc(existingDoc.ref, {
					...
					...input.clothesWeight !== void 0 ? { clothesWeight: input.clothesWeight } : {},
					updatedAt: serverTimestamp()
				});
				return existingDoc.id;
			}
...
async function deleteMessRequest(id) {
	try {
		await import("../_libs/firebase.mjs").then((n) => n.t).then(({ deleteDoc }) => deleteDoc(doc(getDb(), "messRequests", id)));
	} catch (error) {
...
function useLaundryPickupsForStudent(studentId) {
	return useQuery({
		queryKey: [
			"laundryPickups",
			"student",
...
//#endregion
export { upsertLaundryPickup as $, savePackage as A, updateCollege as B, filterByPeriod as C, useProperties as Ct, getOrCreateStudentLaundryRecord as D, getOrCreateMessRecord as E, useUserProfile as Et, setPackageActive as F, updateMessRecordField as G, updateLaundryEmployee as H, todayDateString as I, updatePayoutStatus as J, updateMessRequest as K, todayISTDateString as L, setCollegeActive as M, setLaundryStatus as N, getWeekBounds as O, setMessStatus as P, upsertDelivery as Q, unassignStudentFromLaundry as R, fetchAdmissions as S, usePayoutsByMess as St, generatePayoutId as T, useStudentLaundryRecords as Tt, updateMess as U, updateLaundry as V, updateMessEmployee as W, updateStudentLaundryStatus as X, updateStudentLaundryRecord as Y, updateStudentTiffinStatus as Z, deleteLaundry as _, useMessRequestsForMess as _t, computeStats as a, useDeliveriesForStudent as at, ensureStudentAuthAccount as b, usePackages as bt, createDoNotWantRecord as c, useEmployees as ct, createMess as d, useLaundryEmployees as dt, useAdmissions as et, createMessEmployee as f, useLaundryPickupSummary as ft, deleteAdmission as g, useMessRecordsForStudent as gt, currentISTTime as h, useMessRecordsForDate as ht, assignStudentToMess as i, useDeliveriesForDate as it, setCityActive as j, getWeekId as k, createLaundry as l, useLaundries as lt, createPayout as m, useLaundryPickupsForStudent as mt, addCollege as n, useCities as nt, createAdminUser as o, useDeliverySummary as ot, createMessRequest as p, useLaundryPickupsForDate as pt, updatePayout as q, assignStudentToLaundry as r, useColleges as rt, createAdmission as s, useEmployeeByUid as st, addCity as t, useAllPayouts as tt, createLaundryEmployee as u, useLaundryEmployeeByUid as ut, deleteMess as v, useMessRequestsForStudent as vt, generateAdmissionId as w, useRooms as wt, fetchAdmission as x, usePayoutsByLaundry as xt, deleteMessRequest as y, useMesses as yt, updateAdmission as z };
